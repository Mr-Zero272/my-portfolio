...
import { CheckIcon, ChevronDown, Loader2, Plus, WandSparkles, XCircle, XIcon } from 'lucide-react';
...
interface MultiSelectProps
  ...
  /**
   * If true, allows creating new options when typing in the search input.
   * Optional, defaults to false.
   */
   ...
   * Can return a Promise for async operations (like API calls).
   * Receives the input value and should return a new MultiSelectOption.
   */
  onCreateOption?: (inputValue: string) => Promise<MultiSelectOption> | MultiSelectOption;

  /**
   * Customize the label for the "create new option" suggestion.
   * Receives the current input value and should return a string.
   * Optional, defaults to 'Create "{inputValue}"'.
   */
  createOptionLabel?: (inputValue: string) => string;

  /**
   * If true, shows a loading state when creating a new option.
   * Useful when onCreateOption returns a Promise.
   * Optional, defaults to false.
   */
  ...
  (
    ...
    const handleInputKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (event.key === 'Enter') {
        ...
        // Check if we have filtered options
        let hasFilteredOptions = false;
        if (isGroupedOptions(filteredOptions)) {
          hasFilteredOptions = filteredOptions.some((group) => group.options.length > 0);
        } else {
          hasFilteredOptions = filteredOptions.length > 0;
        }

        // If no filtered options and allowCreateOption is true, create new option
        if (!hasFilteredOptions && allowCreateOption && searchValue.trim() && onCreateOption) {
    ...
    const handleCreateOption = React.useCallback(
      async (inputValue: string) => {
        if (!onCreateOption || !inputValue.trim() || disabled) return;

        setIsCreatingOption(true);
        try {
          const newOption = await onCreateOption(inputValue.trim());

          // Add the new option to selected values
          const newSelectedValues = [...selectedValues, newOption.value];
          setSelectedValues(newSelectedValues);
          onValueChange(newSelectedValues);

          // Clear search and close popover if needed
          setSearchValue('');
          if (closeOnSelect) {
            setIsPopoverOpen(false);
          }
          ...
          announce(`Created and selected "${newOption.label}"`);
        } catch (error) {
          console.error('Failed to create option:', error);
          announce('Failed to create option', 'assertive');
        } finally {
          setIsCreatingOption(false);
        }
      },
      [onCreateOption, selectedValues, onValueChange, closeOnSelect, announce, disabled],
    );
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          >
            <Command>
              ...
              >
                <CommandEmpty
                  className='text-muted-foreground cursor-pointer px-4 py-2'
                  // onSelect={() => handleCreateOption(searchValue)}
                  onClick={() => handleCreateOption(searchValue)}
                >
                  {/* {emptyIndicator || "No results found."} */}
                  {isCreatingOption || creatingOption ? (
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <span>{createOptionLabel ? createOptionLabel(searchValue) : `Create "${searchValue}"`}</span>
                    </div>
                  )}
                </CommandEmpty>
                ...
                {isGroupedOptions(filteredOptions) ? (
                  <>
                    ...
                    {/* Show create option when searching and no results in grouped mode */}
                    {allowCreateOption &&
                      searchValue.trim() &&
                      onCreateOption &&
                      filteredOptions.every((group) => group.options.length === 0) && (
                        <CommandGroup>
                          <CommandItem
                            onSelect={() => handleCreateOption(searchValue)}
                            className='border-muted-foreground/20 bg-muted/30 cursor-pointer border-2 border-dashed'
                            disabled={isCreatingOption || creatingOption}
                          >
                            {isCreatingOption || creatingOption ? (
                              <div className='flex items-center'>
                                <Loader2 className='mr-2 h-4 w-4 animate-spin' />
                              ...
                              <div className='flex items-center'>
                                ...
                                <span>
                                  {createOptionLabel ? createOptionLabel(searchValue) : `Create "${searchValue}"`}
                                </span>
                              </div>
                            )}
                          </CommandItem>
                  ...
                  <CommandGroup>
                    ...
                    {/* Show create option when searching and no results */}
                    {allowCreateOption && searchValue.trim() && onCreateOption && filteredOptions.length === 0 && (
                      <CommandItem
                        onSelect={() => handleCreateOption(searchValue)}
                        className='border-muted-foreground/20 bg-muted/30 cursor-pointer border-2 border-dashed'
                        disabled={isCreatingOption || creatingOption}
                ...
                {/* This section is now handled inline with results above */}
                <CommandSeparator />
                <CommandGroup>
                  <div className='flex items-center justify-between'>
                    {selectedValues.length > 0 && (
