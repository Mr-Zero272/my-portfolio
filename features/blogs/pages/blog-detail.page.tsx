...
export function BlogDetailPage({ post }: PostPreviewFeatureProps) {
  ...
  const createMarkup = useCallback(() => {
    return { __html: DOMPurify.sanitize(post.contentHtml || post.content || '') };
  }, [post.contentHtml, post.content]);

  return (
    <div className="relative min-h-screen bg-background flex-1">
        ...
        <div className="mb-8">
          ...
          <div className="mb-6 flex items-center gap-4">
            <Avatar className="size-8 md:size-12">
              <AvatarImage src={firstAuthor?.user?.image ?? undefined} alt={firstAuthor?.user?.name || 'Anonymous'} />
              <AvatarFallback>
        ...
        {post.featureImage && (
          <div className="mb-8">
            <div className="relative aspect-video w-full overflow-hidden rounded-lg">
              <Image src={post.featureImage?.url ?? undefined} alt={post.title} fill className="object-cover" priority />
            </div>
